...
struct tcp_sock *tcp_sock_accept(struct tcp_sock *tsk)
{
	...
	if(tcp_sock_accept_queue_empty(tsk)){
		//fprintf(stdout, "[accept]:empty, sleep on wait_accept.\n");
		int sleep_result = sleep_on(tsk->wait_accept);
		//fprintf(stdout, "[accept]:wake up on wait_accept.\n");
		if(sleep_result<0){
			return NULL;
		}
	}
...
int tcp_sock_read(struct tcp_sock *tsk, char *buf, int len){
	static int times = 0;
	//fprintf(stdout, "**tcp_sock_read [%d]: into tcp_sock_read.\n", ++times);
	while( ring_buffer_empty(tsk->rcv_buf) ){
		//fprintf(stdout, "**tcp_sock_read: *[sleep_on] wait_recv, buf empty\n");
		if( sleep_on(tsk->wait_recv)<0 ){
			//fprintf(stdout, "**tcp_sock_read: be *[wake_up] ERROR\n");
			return -1;
		}
		//fprintf(stdout, "**tcp_sock_read: be *[wake_up] from wait_recv\n");
	}
	//fprintf(stdout, "**tcp_sock_read: read buf\n");
	int rlen = read_ring_buffer(tsk->rcv_buf, buf, len); //with mutex inside it
	//fprintf(stdout, "**tcp_sock_read: read buf [finish]\n");
	wake_up(tsk->wait_recv);
	//fprintf(stdout, "**tcp_sock_read: *[wake_up] wait_recv\n");
	return rlen;
}
...
int tcp_sock_write(struct tcp_sock *tsk, char *buf, int len){
	...
	//fprintf(stdout, "**tcp_sock_write [%d]: into tcp_sock_write.\n",++times);
	while(tsk->snd_una - init_seq < data_len){
		int snd_len = (init_seq +len >= tsk->snd_una+ TCP_DEFAULT_WINDOW)? TCP_DEFAULT_WINDOW: init_seq +len -tsk->snd_una;
		tcp_send_data(tsk, buf+(tsk->snd_una -init_seq), snd_len);
		//fprintf(stdout, "**tcp_sock_write: *[sleep_on] wait_send, have sent tcp data.\n");
		if( sleep_on(tsk->wait_send)<0 ){
			//fprintf(stdout, "**tcp_sock_write: be *[wake_up] ERROR\n");
			return -1;
		}
		//fprintf(stdout, "**tcp_sock_write: be *[wake_up] from wait_send\n");
	}
	return 0;
}
