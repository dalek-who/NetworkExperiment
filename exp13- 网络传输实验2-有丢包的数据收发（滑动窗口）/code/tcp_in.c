...
void tcp_state_listen(struct tcp_sock *tsk, struct tcp_cb *cb, char *packet)
{
	...
	tcp_set_retrans_timewait_timer(csk);
...
void tcp_state_syn_sent(struct tcp_sock *tsk, struct tcp_cb *cb, char *packet)
{
	...
	if( (cb->flags & (TCP_SYN|TCP_ACK)) == (TCP_SYN|TCP_ACK) ){ //todo 3
...
static inline int is_tcp_seq_valid(struct tcp_sock *tsk, struct tcp_cb *cb)
{
	...
	if (less_than_32b(cb->seq, rcv_end) && less_or_equal_32b(tsk->rcv_nxt, cb->seq_end)) {
		...
		log(ERROR, "received packet with invalid seq=%d, drop it.",cb->seq);
		return 0;
	}
}
...
/*	
	...
	*/
	...
	ofo_packet_dequeue(tsk); //change tsk->snd_una inside it
	...
	*/
	...
	if((cb->flags & TCP_ACK) == 0){ //todo 3
		...
		//      (if it ACKs new data, update the window;)
			...
			if(cb->pl_len==0 || strcmp(cb->payload,"666-get")==0){ //just an ACK packet
				tsk->snd_una = cb->ack;
				tsk->rcv_nxt = cb->seq +1;
